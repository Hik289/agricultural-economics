...
def main() -> int:
    ...
    report.append(f"- Models used: deterministic rules plus configured primary and secondary model-assisted coders. Actual per-case secondary model summary: {secondary_model_summary or '<none recorded>'}.")
    report.append("")
    ...
    report.append("")
    report.append("## 2. Three-way (rule / primary model / secondary model) per-indicator agreement")
    report.append("")
    ...
    report.append(f"- Cases where the primary and secondary model coders agree on **every** one of the 12 indicators: {full_agree_rows} of {n} ({100*full_agree_rows/n:.1f}%).")
    ...
    report.append("- The per-indicator pattern should be interpreted as a consistency check among the deterministic rule baseline, the primary model coder, and the secondary model coder.")
    report.append("")
    report.append("## 9. Notes & limitations")
    report.append("")
    report.append("- **Model configuration.** Optional model-assisted coding depends on the configured primary endpoint and, when used, the configured secondary proxy. Re-running with different models or endpoints should be treated as a new coding condition.")
